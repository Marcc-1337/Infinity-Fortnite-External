...
public:
	vector3 get_bone_3d(int id) const {
		...
		auto bone_transform = driver.read<FTransform>(driver.read<u>((u)this + 0x10 * is_cached + 0x610) + 0x60 * id);

		FTransform ComponentToWorld = driver.read<FTransform>((u)this + 0x240);
	...
	float get_last_submit_time() const {
		...
		return driver.read<float>((u)this + 0x360);
	}
...
public:
	...
	ugameinstance* get_game_instance() const {
		if (!(u)this) return 0;
		return driver.read<ugameinstance*>((u)this + 0x1b8);
	}
